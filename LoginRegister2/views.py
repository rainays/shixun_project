    ...
    if request.method == 'POST':
        ...
        if stm not in obj:
            ...
            if check_password(pwd, obj1.password):
                ...
                #check = request.POST['checkedd']
                check = request.POST.get('checkedd', None)
                if (check == 'RememberMe'):
...
def picupload(request):
    ...
    if request.POST:
        ...
        if '.png' in allowedTypes:
            ...
            uploadDirPath = os.path.join(os.getcwd(), 'media',username)
...
def change(request):
    ...
    print('旧名字',oldname)
    if request.method == 'POST':
         newusername = request.GET['newname']
         obj = models.User.objects.get(name=oldname)
         obj.nickname=newusername
         print('新名字',obj.nickname)
         obj.save()
    #userinfo()     #stu = HttpResponse("/userinfo/")
...
def mypage(request):
  ...
  if t != " ":
    ...
    user = models.User.objects.get(name=username)     ##############user
   ...
   # filepath = []
    for file in context['ShowFiles']:
        ...
        if fileType in Types1:
            ...
            pic.append({'path':imgPath, 'file':file,'filepath':path1})
        elif fileType in Types2:
            imgPath = 'icons'+ '/' + 'doc.png'
            office.append({'path':imgPath, 'file':file,'filepath':path1})
        elif fileType in Types3:
            imgPath = 'icons'+ '/' + 'xls.png'
            office.append({'path':imgPath, 'file':file,'filepath':path1})
        elif fileType in Types4:
            imgPath = 'icons' + '/' + 'video.png'
            video.append({'path':imgPath, 'file':file,'filepath':path1})
        else:
            imgPath = 'icons' + '/' + 'others.png'
            other.append({'path':imgPath, 'file':file,'filepath':path1})
        imgPath.replace('\\', '/')
        pathlist.append ({'path':imgPath, 'file':file,'filepath':path1})
...
def ToShare(request):
    # print(890809809980)
    if request.method == 'GET':
        filename = request.GET.get('moddal', None)
        username = request.COOKIES.get('name', None)

        link = username + '/' + filename
        link = base64.b64encode(link.encode(encoding="utf-8")).decode()
        link = 'http://127.0.0.1:8000/mycloud/Share/' + link
        print('='*50)
        print(link)
        #return mypage(request,link)
